...
resetState = do
    liftIO $ writeIORef defaultCheckerState
        CheckerState
...
instantiateAndSub :: MonadIO m => HLIR.Scheme HLIR.Type -> m (HLIR.Type, Map Text HLIR.Type)
...
instantiateMapAndSub :: MonadIO m => HLIR.Scheme (Map Text HLIR.Type) -> m (Map Text HLIR.Type, Map Text HLIR.Type)
...
instantiateMap :: MonadIO m => HLIR.Scheme (Map Text HLIR.Type) -> m (Map Text HLIR.Type)
...
applySubstitution s (HLIR.MkTyAnonymousStructure fields) = do
    fields' <- Map.traverseWithKey (\_ ty -> applySubstitution s ty)
        fields
